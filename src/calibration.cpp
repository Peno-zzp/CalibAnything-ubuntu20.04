...
#include <random>  // 提供 std::default_random_engine 和 std::uniform_real_distribution
#include <ctime>   // 提供 clock() 和 time()
#include <chrono>
#include <sstream>
...
std::string  Calibrator::timeStr(){
    auto now = std::chrono::system_clock::now();
    std::time_t now_time = std::chrono::system_clock::to_time_t(now);
    std::tm *local_time = std::localtime(&now_time); // 本地时区时间
...
void Calibrator::Calibrate()
{
    ...
    VisualProjectionSegment(extrinsic_, timeStr()+"init_proj_seg.png", 0);
    VisualProjection(extrinsic_, timeStr()+"init_proj.png", 0);
    if (params_.is_gt_available){
        VisualProjectionSegment(params_.extrinsic_gt, timeStr()+"gt_proj_seg.png", 0);
        VisualProjection(params_.extrinsic_gt, timeStr()+"gt_proj.png", 0);
    }
    ...
    VisualProjectionSegment(extrinsic_, timeStr()+"refined_proj_seg.png", 0);
    VisualProjection(extrinsic_, timeStr()+"refined_proj.png", 0);
}
